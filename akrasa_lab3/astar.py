	...
	def Children(self):
		...
		for i in range(0, height): #height should be set to hieght of grid
			for j in range(0, width): #width should be set to width of grid
				...
				print "I reach conditions:"	
				if (self.grid.data[i*width+j] == 100):
					pass
				elif(x+1 == i and y == j):
					print 2
					child1 = Node(v+1, (x+1, y), s, g)
					self.children.append(child1)
				elif(x == i and y+1 == j):
					print 3
					child2 = Node(v+1, (x, y+1), s, g)
					self.children.append(child2)
				elif(x-1 == i and y == j):
					print 4
					child3 = Node(v+1, (x-1, y), s, g)
					self.children.append(child3)
				elif(x == i and y-1 == j):
					print 5
					child4 = Node(v+1, (x, y-1), s, g)
...
def aStar(start, goal, grid):
	...
	origin.CalF()
	print "stuck on children"
	origin.Children()
	print "nope"
	print origin.children[0].CalF()
