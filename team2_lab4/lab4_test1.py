...
def gridToWorld(gridPoint, worldMap):
    res = worldMap.info.resolution
    worldPoint = Point()
    worldPoint.x = ((gridPoint.x*res) + worldMap.info.origin.position.x) + (0.5*res)
    worldPoint.y = ((gridPoint.y*res) + worldMap.info.origin.position.y) + (0.5*res)
    return worldPoint
...
def genWaypoints(g_path_rev, worldMap):
    ...
    for point in gridPoints:
        worldPoints.append(gridToWorld(point, worldMap))
    # Create actual Path()
    path = Path()
